...
async function AdminGetUserToDoDeleted(req: Request, res: Response) {
  try {
    ...
    const totalCount = await ToDoDeleted.countDocuments({ userId });
    const pages = Math.ceil(totalCount / limit) || 1;
    let skip;
    if (pages == 1) skip = 0;
    else skip = (page - 1) * limit;
    const todos = await ToDoDeleted.find({ userId })
      .sort({ date: -1 })
      .skip(skip)
      .limit(limit);
