...
async function ToDoCompletedGet(req: Request, res: Response) {
  try {
    ...
    const totalCount = await ToDoCompleted.countDocuments({
    ...
    const todos = await ToDoCompleted.find({ userId: decodedToken.id })
      .sort({ date: -1 })
      .skip(skip)
      .limit(limit);
